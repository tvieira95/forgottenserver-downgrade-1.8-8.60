...
function spell.onCastSpell(creature, variant)
	...
	if not target or target:getGroup():getAccess() and
	...
	local maxPositionDifference, direction = math.max(
		                                         math.abs(positionDifference.x),
		                                         math.abs(positionDifference.y))
	if maxPositionDifference >= 5 then
		local positionTangent = positionDifference.x ~= 0 and positionDifference.y /
		...
		elseif math.abs(positionTangent) < 2.4142 then
			direction = positionTangent > 0 and
				            (positionDifference.y > 0 and DIRECTION_NORTHWEST or
					            DIRECTION_SOUTHEAST) or positionDifference.x > 0 and
	...
	local level =
		positionDifference.z > 0 and LEVEL_HIGHER or positionDifference.z < 0 and
			LEVEL_LOWER or LEVEL_SAME
	local distance = maxPositionDifference < 5 and DISTANCE_BESIDE or
		                 maxPositionDifference < 101 and DISTANCE_CLOSE or
		                 maxPositionDifference < 250 and DISTANCE_FAR or
		                 DISTANCE_VERYFAR
	...
	creature:sendTextMessage(MESSAGE_INFO_DESCR,
...
spell:vocation("druid", "elder druid", "paladin", "royal paladin", "sorcerer", "master sorcerer", "knight", "elite knight", "monk", "exalted monk", "cleric", "hierophant", "ranger", "huntsman", "runelord", "archmage", "warlord", "dreadlord", "soulwarden", "astral master")
spell:register()
